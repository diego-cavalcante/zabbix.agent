...
# Update  = Versão 1.9 11/12/2017 (Contribuição Mário Alves - ADD parâmetros para instalação via compartilhamento de rede e validações).
...
### 1º Parte IF VERIFICA_SO = 1 = Windows 64 Bits ##########################################################
   ...
   Write-Host "                                Update 11/12/2017 - Versão 1.9                              #"
   Write-Host "#############################################################################################"
   Write-Host ""
...
### ELSE VERIFICAVERSION = 1 = ENCONTRADO ##################################################################
   ...
   Write-Host "Ex: c:\zabbix ou c:\monitor                                                                  "
...
### PREENCHENDO .CONF COM AS INFRMAÇÕES DIGITADAS ##########################################################
   ...
   Write-Host "#############################################################################################"
   Write-Host ""
   powershell.exe c:\$PASTA\zabbix_agentd.exe -i -c c:\$PASTA\zabbix_agentd.conf
   powershell.exe c:\$PASTA\zabbix_agentd.exe -s -c c:\$PASTA\zabbix_agentd.conf
   Start-Sleep -s 3
   Write-Host ""
   Write-Host "#############################################################################################"
...
### ELSE VERIFICA_PASTA = 1 = ENCONTRADA ################################################################### 
   ...
   powershell.exe c:\$PASTA\zabbix_agentd.exe -d -c c:\$PASTA\zabbix_agentd.conf
   Write-Host ""
   Write-Host "#############################################################################################"
   ...
   Write-Host "E não se preocupe, um backup de c:\$PASTA foi salvo em c:\$PASTA.backup.$DATAATUAL"
   Write-Host "#############################################################################################"
   Write-Host ""
   cd c:\agent.zabbix
 }
...
### ELSE INSTALLVERSION = 1 = ENCONTRADO ###################################################################   
   ...
   Write-Host ""
   Write-Host "#############################################################################################"
   Write-Host "Backup realizado com sucesso, continuando instalação...                                      "
   Write-Host "#############################################################################################"
   Write-Host ""
   Write-Host "#############################################################################################"
   ...
   Write-Host "Backup: Não se preocupe, um backup foi salvo em c:\$PASTA.backup.$DATAATUAL                  "
   Write-Host "                                                                                             "
   Write-Host "#############################################################################################"
   Write-Host ""
   Write-Host "#############################################################################################"
   ...
   Write-Host ""
   cd c:\agent.zabbix
#  Remove-Item c:\$PASTA.backup.$DATAATUAL -Recurse
 }
}
